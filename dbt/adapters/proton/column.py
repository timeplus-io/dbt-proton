...
Self = TypeVar('Self', bound='ProtonColumn')


@dataclass
class ProtonColumn(Column):
    TYPE_LABELS = {
        'STRING': 'string',
        'TIMESTAMP': 'datetime',
        'FLOAT': 'float64',
    ...
    def __repr__(self) -> str:
        return f'<ProtonColumn {self.name} ({self.data_type}, is nullable: {self.is_nullable})>'

    @property
    def data_type(self) -> str:
        if self.is_string():
            return ProtonColumn.string_type(self.string_size())
        elif self.is_numeric():
            return ProtonColumn.numeric_type(
